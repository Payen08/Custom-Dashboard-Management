...
└── TableSurface
    ├── Toolbar：搜索、筛选、更多筛选、结果数
    ├── TableScroll：表头＋数据行
    └── Pagination：总数、页码、每页数量
```

- Header 标题 20px / 600；说明 12px；同一区域最多一个 Primary。
- Toolbar 控件高 40px；搜索建议宽 312px；结果数靠右。
- Current 表头高 44px，数据行高 60px，单元格水平内边距 16px。
- 主标识可使用文本链接；同单元格次级说明使用 11–12px Muted 文本。
- 操作列默认固定右侧，基准宽度 96px；操作组使用 32px `ProductIconButton`，间距 8px。
...
- Header 显示任务标题与一句说明；Footer 固定，取消在左、唯一 Primary 在右。
- Body 是唯一滚动区；表单默认两列，字段间距 20px，分组间距 24px。
- 分组标题 14px / 600；分组之间使用一条 `border` 分隔线。
- 字段必须由 `ProductField` 组织，顺序为 Label → Control → Hint/Error。
- 关闭存在未保存修改的表单前必须确认；保存 Loading 不得清空字段或改变按钮宽度。

## 4. Drawer 详情

- 查看详情使用 `ProductDrawer`；默认 420px，复杂详情可使用 `min(760px, calc(100vw - 48px))`。
- Header 显示对象标识和摘要；状态 Tag 放在 Body 首组或 Header 描述附近。
- Body 按“基础信息、业务信息、明细、操作日志”分组，只允许 Body 滚动。
- Footer 默认放关闭；可编辑对象可增加次要操作和唯一 Primary 编辑操作。
- Drawer 内进入执行追踪等二级详情时，保留 Drawer 外壳并提供明确返回入口，不再叠加第二个 Drawer。
- 抽屉关闭后焦点回到触发按钮，列表查询、滚动位置与分页保持不变。
