...
`tokens/design-tokens.json` 是前端接入的唯一 Token 源；它给出了 light / dark 两套完整语义色、阴影，以及共享的字体、间距、圆角、透明度、动效、层级、布局、控件、状态和组件 Token。

`adapters/ant-design-theme.ts` 是 Ant Design 主题映射示例。应用根部按当前主题模式调用 `createProductTheme(mode)` 并传入主题提供者即可；业务页面不得再写颜色、间距、圆角、阴影、控件高度或动效时长。
