...
function scopeVars(scope: ArcoScope): CssVars {
  ...
  return {
    ...
    '--arcoui-accent': `var(--${prefix}-accent)`,
    '--arcoui-accent-text': scope === 'robot' ? 'var(--robot-accent-text)' : `var(--${prefix}-accent)`,
    ...
    '--arcoui-overlay': scope === 'robot' ? 'var(--robot-overlay)' : 'rgba(0, 0, 0, 0.4)',
    '--arcoui-shadow': scope === 'robot' ? 'var(--robot-dialog-shadow)' : '0 24px 80px -28px rgba(15, 23, 42, 0.38), 0 10px 30px -18px rgba(15, 23, 42, 0.24)',
  };
}
