...
import tokens from '../../../guidelines/design-tokens.json';
import componentSpecs from '../../../guidelines/component-specs.json';
import guidelinesMarkdown from '../../../guidelines/Guidelines.md?raw';
import {
  ProductButton,
  ProductCheckbox,
  ProductField,
...
const GUIDE_GROUPS: Array<{ label: string; items: Array<{ key: string; label: string; section: GuideSection }> }> = [
  { label: '基础', items: [
    ...
    { key: 'icons', label: '图标', section: 'components' }, { key: 'spacing', label: '间距', section: 'tokens' },
    { key: 'radius', label: '圆角', section: 'tokens' }, { key: 'shadow', label: '阴影', section: 'tokens' },
  ...
  { label: '组件', items: [
    { key: 'general', label: '通用', section: 'components' }, { key: 'navigation', label: '导航', section: 'components' },
...
const GUIDELINE_SECTION_MAP: Record<string, GuidelineExcerpt[]> = {
  ...
  icons: [{ marker: '### 11.4 表单、图标操作与反馈', title: '图标操作与反馈' }],
  ...
  general: [{ marker: '## 3. 基础组件', title: '基础组件' }, { marker: '### 3.1 组件使用与视觉契约', title: '组件使用与视觉契约' }, { marker: '### 13.1 Button 与 IconButton', title: 'Button 与 IconButton' }],
  navigation: [{ marker: '### 13.4 Tabs 与 Menu', title: 'Tabs 与 Menu' }, { marker: '### 11.3 展开、选择与筛选', title: '展开、选择与筛选' }],
  'data-entry': [{ marker: '## 14. 表单规范', title: '表单规范' }, { marker: '### 13.2 Input 与 Select', title: 'Input 与 Select' }, { marker: '### 13.3 Checkbox', title: 'Checkbox' }],
...
type TopicDocument = {
  ...
  rules: string[];
  preview: 'colors' | 'type' | 'spacing' | 'radius' | 'shadow' | 'motion' | 'layout' | 'document';
};
...
const TOKEN_DOCUMENTS: Record<string, TopicDocument> = {
  ...
  typography: { title: '字体', description: '字体、字号、字重和行高均从已发布 Token 选择，保证中英文与数字在不同页面一致。', parameterTitle: '排版 Token', parameters: [{ token: '--ds-font-family-sans', value: 'PingFang SC / Noto Sans SC / Microsoft YaHei' }, { token: '--ds-font-size-12', value: '12px' }, { token: '--ds-font-size-14', value: '14px' }, { token: '--ds-font-size-16', value: '16px' }, { token: '--ds-font-size-18', value: '18px' }, { token: '--ds-line-height-normal', value: '1.5' }], rules: ['正文默认使用 14px；标题层级仅使用已发布字号。', '长文本允许换行或截断，但核心数值与单位不能分离。', '不得引入页面私有字体。'], preview: 'type' },
  ...
  shadow: { title: '阴影', description: '阴影只用于表达浮层和必要的层级；常规卡片不叠加额外阴影。', parameterTitle: '阴影 Token', parameters: [{ token: '--ds-shadow-none', value: 'none' }, { token: '--ds-shadow-xs', value: '轻层级' }, { token: '--ds-shadow-card', value: '强调卡片' }, { token: '--ds-shadow-overlay', value: 'Popover / Select' }, { token: '--ds-shadow-dialog', value: 'Modal / Dialog' }], rules: ['业务卡片优先使用描边，不用投影制造层级。', 'Modal 只使用 dialog 阴影 Token，不能在业务页重写。', '阴影不承担 Hover 的唯一反馈。'], preview: 'shadow' },
  ...
  'design-tokens': { title: 'Design Token', description: 'Token JSON 是前端接入的唯一机器可读来源；点击右上角可复制完整 JSON。', parameterTitle: '交付入口', parameters: [{ token: 'guidelines/design-tokens.json', value: `v${tokens.version}` }, { token: 'guidelines/ant-design-theme.ts', value: '主题映射' }, { token: 'src/styles/design-system.css', value: 'CSS Token 入口' }], rules: ['主题切换只切换 Token 集合，不改变页面结构、字号或控件尺寸。', '新增全局样式前必须先发布 Token。'], preview: 'document' },
...
export function DesignGuidelines({ themeMode }: { themeMode: ThemeMode }) {
  ...
  const visibleComponentSpecs = componentSpecs.components.filter(component => componentCategory === 'all' || component.category === componentCategory);
    ...
    <main className="ds-page ds-page--dashboard ds-guidelines" aria-label="设计规范">
      <div className="ds-guidelines__body">
        ...
        <section className="ds-guidelines__content" aria-label={`${activeTopic.label}规范`}>
          <GuideHeading title={tokenDocument.title} description={tokenDocument.description} />
          <DocumentRules rules={tokenDocument.rules} />
          {tokenDocument.preview === 'colors' && <ColorTokenGroups onCopy={copy} />}
          <TokenList title={tokenDocument.parameterTitle} description="点击参数复制 CSS Token 或交付引用。" items={tokenDocument.parameters} onCopy={copy} wide />
...
function GuidelineSourceContent({ excerpts }: { excerpts: Array<GuidelineExcerpt & { content: string }> }) {
  ...
  return (
    <section className="ds-guidelines__source" aria-label="规范原文">
      <div className="ds-guidelines__source-heading"><div><h2>文档规则</h2><p>以下内容直接读取 <code>Guidelines.md</code> 的对应章节。</p></div><code>Guidelines.md</code></div>
...
function GuidelineTopicChecklist({ topicKey, title }: { topicKey: string; title: string }) {
  return (
    <section className="ds-guidelines__content" aria-label={`${title}落地检查`}>
      <GuideHeading title={`${title}落地检查`} description="只列出当前目录需要在页面或流程中验证的规则；具体说明以上方文档原文为准。" />
      <GuidelineVisualPreview topicKey={topicKey} title={title} />
      <DocumentRules rules={getTopicRules(topicKey)} />
    </section>
  );
}
...
function GuidelineVisualPreview({ topicKey, title }: { topicKey: string; title: string }) {
  ...
  if (topicKey === 'batch') return <section className="ds-guidelines__pattern-preview"><h2>样式预览</h2><p>只在选中对象后出现批量操作栏，并明确展示影响数量。</p><div className="ds-guidelines__pattern-surface ds-guidelines__pattern-surface--list"><div className="ds-guidelines__pattern-batchbar"><span>已选 3 项</span><div><ProductButton size="small">导出</ProductButton><ProductButton size="small" type="outline">变更状态</ProductButton><ProductButton size="small" type="primary" status="danger">删除</ProductButton></div></div><PreviewRows selectable labels={['人形双足机器人', '四足巡检机器人', '机械臂控制器']} /></div></section>;
  ...
  if (topicKey === 'delete-confirm') return <section className="ds-guidelines__pattern-preview"><h2>样式预览</h2><p>确认层只说明对象、影响范围与后果；危险确认使用 danger，取消不改变数据。</p><div className="ds-guidelines__pattern-dialog"><h3>删除型号</h3><p>删除后，“人形双足机器人”及其未发布配置将无法恢复。确定要删除吗？</p><div><ProductButton>取消</ProductButton><ProductButton type="primary" status="danger">确认删除</ProductButton></div></div></section>;
...
function PreviewRows({ labels, selectable = false }: { labels: string[]; selectable?: boolean }) {
  return <div className="ds-guidelines__pattern-rows">{labels.map((label, index) => <div key={label}>{selectable && <ProductCheckbox aria-label={`选择${label}`} defaultChecked={index < 2} />}<strong>{label}</strong><ProductTag tone={index === 1 ? 'success' : 'neutral'}>{index === 1 ? '已发布' : '未发布'}</ProductTag><span>2026-07-21</span><ProductButton type="text" size="small">查看</ProductButton></div>)}</div>;
}
...
function ComponentContractExplorer({ topic, components, activeComponent, onSelect, onCopy, showExamples }: {
  topic: string;
  ...
  return (
    <section className="ds-guidelines__content ds-guidelines__contracts" aria-label={`${topic}组件规范`}>
      ...
      <article className="ds-guidelines__contract" aria-label={`${activeComponent.name}组件契约`}>
...
function ComponentStylePreview({ componentName }: { componentName: string }) {
  ...
  if (componentName === 'Menu / Dropdown') return <div className="ds-guidelines__component-menu"><button type="button">编辑</button><button type="button">复制链接</button><button type="button" disabled>导出中</button><hr /><button type="button" data-danger="true">删除</button></div>;
  ...
  if (componentName === 'Table') return <div className="ds-guidelines__component-table"><header><span>型号名称</span><span>发布状态</span><span>更新时间</span><span>操作</span></header><div><strong>人形双足机器人</strong><ProductTag tone="success">已发布</ProductTag><span>2026-07-21</span><ProductButton type="text" size="small">查看</ProductButton></div><div data-selected="true"><strong>四足巡检机器人</strong><ProductTag tone="accent">已选择</ProductTag><span>2026-07-20</span><ProductButton type="text" size="small">查看</ProductButton></div></div>;
  if (componentName === 'Modal') return <div className="ds-guidelines__component-modal"><header><strong>取消发布型号</strong><ProductIconButton icon={<X size={16} />} aria-label="关闭" /></header><p>取消发布后，型号将恢复为可编辑状态，后续修改不会同步到已发布版本。</p><footer><ProductButton size="small">取消</ProductButton><ProductButton size="small" type="primary">确认取消发布</ProductButton></footer></div>;
  if (componentName === 'Drawer') return <div className="ds-guidelines__component-drawer"><header><strong>参数配置</strong><ProductIconButton icon={<X size={16} />} aria-label="关闭" /></header><div><span>运行模式</span><ProductSelect defaultValue="auto"><option value="auto">自动</option><option value="manual">手动</option></ProductSelect></div><footer><ProductButton size="small">取消</ProductButton><ProductButton size="small" type="primary">保存</ProductButton></footer></div>;
  ...
  if (componentName === 'Toast / Notification') return <div className="ds-guidelines__component-toast-list"><div data-tone="success"><Check size={16} /><span><strong>保存成功</strong><small>型号配置已保存到草稿</small></span><ProductIconButton icon={<X size={14} />} aria-label="关闭提示" /></div><div data-tone="danger"><span>!</span><span><strong>保存失败</strong><small>请检查网络后重试</small></span><ProductButton type="text" size="small">重试</ProductButton></div></div>;
...
function getTopicRules(topicKey: string) {
  const rules: Record<string, string[]> = {
    icons: ['图标使用统一图标库与已发布尺寸；纯图标操作必须有 aria-label 和 Tooltip。', '图标不使用装饰性圆形底框来表达状态。'],
    navigation: ['当前项使用 aria-selected 或 aria-current，不使用模糊的 Active 状态命名。', 'Tab 支持左右方向键，空间不足时横向滚动或收纳。'],
    'data-entry': ['Input、Textarea、Select 必须有 Label；Placeholder 不能替代字段名。', 'Checkbox 正常 Checked 使用品牌色底与反白勾选；Disabled 才可使用灰色勾。'],
    'data-display': ['Table 的 Header、Row Hover、Selected、Loading、Empty、Error 状态必须完整。', '表格横向溢出只在表格内部滚动。'],
    'search-filter': ['搜索 + 筛选 + 结果列表保留已生效筛选与结果数。', '无结果保留筛选并提供清除入口；失败提供重试。'],
    'form-submit': ['提交 Loading 阻止重复操作；成功后的停留、关闭或跳转由页面明确说明。', '未保存离开必须确认。'],
    batch: ['仅选中至少一项后显示批量操作栏。', '危险批量操作必须确认并显示影响数量。'],
    import: ['导入先校验再确认写入；上传、解析、失败和重试逐项反馈。', '单个文件失败不能丢失其他文件。'],
    'delete-confirm': ['删除确认必须说明对象、影响范围与不可逆后果。', '确认按钮使用 Danger，取消不改变数据。'],
    'long-task': ['展示任务名称、阶段、进度、耗时和后台入口。', '失败说明可恢复动作，完成给出结果入口。'],
    'list-page': ['标题区、筛选区、结果区、分页位置与空/错/加载状态按列表页模板执行。'],
    'detail-page': ['标题区包含对象状态与主要操作；详情内容与侧栏分别管理内部滚动。'],
    'config-page': ['配置页使用持续配置 / 发布模式；提交区固定在内容底部。'],
    'form-page': ['表单字段按 Label、Control、Help、Error 的固定顺序组织。'],
    'dashboard-page': ['看板按 12 列栅格布局；卡片按容器尺寸收敛，不整体缩放。'],
    'editor-page': ['编辑器使用资源、画布、属性三栏；画布操作不作为页面级浮层。'],
    other: ['未覆盖的组件仍必须遵守 Token、状态矩阵、可访问性与响应式规则。'],
  };
  return rules[topicKey] ?? ['当前内容来自已发布设计规范；页面不得新增同义颜色、尺寸、圆角、阴影或层级。'];
}

function TokenStylePreview({ kind }: { kind: TopicDocument['preview'] }) {
  if (kind === 'colors' || kind === 'document') return null;
  if (kind === 'type') return <section className="ds-guidelines__style-preview"><span style={{ fontSize: 'var(--ds-font-size-18)', fontWeight: 'var(--ds-font-weight-semibold)', color: 'var(--ds-color-heading)' }}>型号配置标题</span><span style={{ fontSize: 'var(--ds-font-size-14)' }}>正文信息与参数说明</span><code>SFMono-Regular · 12px</code></section>;
  ...
  if (kind === 'shadow') return <section className="ds-guidelines__style-preview"><strong>阴影样式预览</strong><div className="ds-guidelines__shadow-preview"><i /><i /></div></section>;
