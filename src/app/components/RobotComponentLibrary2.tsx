...
export function RobotComponentLibrary2({ themeMode = 'light', stylePreset = 'current', industrialColorTheme = 'steel', onEditingChange }: { themeMode?: ThemeMode; stylePreset?: StylePreset; industrialColorTheme?: IndustrialColorTheme; onEditingChange?: (editing: boolean) => void }) {
  ...
  return <div className="urdf-library" style={themeVars}>
    ...
    {!editing ? <main className="ds-page ds-page--list urdf-list robot-library-list-page">
      <header className="urdf-list__head robot-library-list-header">
        <div className="robot-library-list-heading"><h1><AdaptiveText copy={{ standard: ui.title, short: ui.titleShort }} style={{ display: 'block', height: 30, maxWidth: 360, whiteSpace: 'nowrap', overflow: 'hidden' }} /></h1><p><AdaptiveText copy={{ standard: ui.description, short: ui.descriptionShort }} style={{ display: 'block', height: 22, maxWidth: 620, whiteSpace: 'nowrap', overflow: 'hidden' }} /></p></div>
      ...
      <div className="urdf-workspace">
        <div className="urdf-left-sidebar">
          <aside className="urdf-panel urdf-assets-panel"><header className="urdf-panel__head"><h2>{e.assets}</h2><ArcoIconButton scope="robot" size="small" icon={<Search size={15}/>} aria-label={e.search} aria-expanded={assetSearchOpen} title={e.search} onClick={() => { setAssetSearchOpen(current => !current); if (assetSearchOpen) setAssetQuery(''); }}/><ArcoIconButton scope="robot" size="small" icon={<FileUp size={15}/>} aria-label={e.uploadAsset} title={e.uploadAsset} onClick={() => glbRef.current?.click()}/></header>{assetSearchOpen && <label className="urdf-tree-search"><Search size={14}/><ArcoTextInput scope="robot" autoFocus value={assetQuery} onChange={event => setAssetQuery(event.target.value)} placeholder={e.searchFiles} aria-label={e.searchFiles} /></label>}<div className="urdf-asset-list">{filteredAssets.length ? filteredAssets.map(asset => <div key={asset.name} className="urdf-asset-row" data-selected={previewAsset === asset.name} draggable onDragStart={event => { event.dataTransfer.effectAllowed = 'copy'; event.dataTransfer.setData('application/x-robot-asset', asset.name); }} onDoubleClick={() => attachAssetToLink(asset.name)}><button type="button" className="urdf-asset-row__main" onClick={() => setPreviewAsset(asset.name)}><span className="urdf-asset-row__icon"><FileBox size={15}/></span><span className="urdf-asset-row__text"><strong>{asset.name}</strong><small>{asset.size}</small></span></button><div className="urdf-asset-row__actions"><ArcoIconButton scope="robot" type="text" size="mini" icon={<Plus size={13}/>} aria-label={`${e.addToLink}: ${asset.name}`} title={e.addToLink} onClick={() => attachAssetToLink(asset.name)}/><ArcoIconButton scope="robot" type="text" size="mini" icon={<Trash2 size={13}/>} aria-label={`${e.deleteAsset}: ${asset.name}`} title={e.deleteAsset} onClick={() => removeAsset(asset.name)}/></div></div>) : <div className="urdf-panel-empty urdf-panel-empty--compact"><FileBox size={20}/><strong>{e.noAssets}</strong><span>{e.assetHelp}</span><ArcoButton scope="robot" size="small" onClick={() => glbRef.current?.click()}>{e.uploadMesh}</ArcoButton></div>}</div></aside>
          <aside className="urdf-panel urdf-structure-panel"><header className="urdf-panel__head"><h2>{e.structure}</h2><ArcoIconButton scope="robot" size="small" icon={<Search size={15}/>} aria-label={e.search} aria-expanded={structureSearchOpen} title={e.search} onClick={() => { setStructureSearchOpen(current => !current); if (structureSearchOpen) setQuery(''); }}/><ArcoIconButton scope="robot" size="small" icon={<Plus size={16}/>} aria-label={e.addNode} title={e.addNode} /></header>{structureSearchOpen && <label className="urdf-tree-search"><Search size={14}/><ArcoTextInput scope="robot" autoFocus value={query} onChange={event => setQuery(event.target.value)} placeholder="Link / Joint" aria-label={e.search} /></label>}<div className="urdf-tree" role="tree">{filteredNodes.map((node) => {
        ...
        <section className="urdf-scene">
          ...
          <div className="urdf-scene__tools" role="toolbar" aria-label={e.sceneTools}><ArcoIconButton scope="robot" type="text" size="small" icon={<Maximize2 size={15}/>} aria-label={e.fit} title={e.fit} onClick={() => setResetViewToken(current => current + 1)}/><ArcoIconToggleButton scope="robot" size="small" selected={showGrid} icon={<Grid3X3 size={15}/>} aria-label={showGrid ? e.hideGrid : e.showGrid} title={showGrid ? e.hideGrid : e.showGrid} onClick={() => setShowGrid(current => !current)}/><ArcoIconToggleButton scope="robot" size="small" selected={interfaceOpen} icon={<RadioTower size={15}/>} aria-label={e.jointInterface} title={e.jointInterface} onClick={() => setInterfaceOpen(current => !current)}/></div>
